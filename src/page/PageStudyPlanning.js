...
class PageStudyPlanningTabClasses{
    ...
    LoadData(){
        ...
        if(this.m_AjaxClasses != null)
            this.m_AjaxClasses.abort();        
        ...
        .always((dt, e, l) => {
            if(this.m_Page == 1)
                ...
                this.HideTableRowLoad();


            this.m_AjaxClasses = null;

            if(Array.isArray(dt["data"]) && dt["data"].length > 0)
                this.m_Data = dt["data"];
            
            if(dt["data"].length == 0 || dt["data"].length < this.m_PageSize)
    ...
    OnButtonClick(el){
        ...
        console.log(action);
        if(action == "add")
            this.EventOpenEditClass();
        else if(action == "add" || this.m_SelectedClass)
            this.EventOpenEditClass(this.m_SelectedClass);
    }
...
class PageStudyPlanningTabEditClass{
    constructor(){
        this.m_Data = null;
        this.m_DataSchemeType = new Array();
    }
    ...
    LoadDate(classId){
        ...
        .always((dt, e, l) => {
            ...
            this.m_Data["arr_user"].forEach(item => {
                var arr = this.m_Data["arr_team"].filter(itemF => item["arr_team"] == itemF["arr_team"]);
                if(arr.length == 0 && item["team"])
                    this.m_Data["arr_team"].push({ "team": item["team"] });
                ...
                if(Array.isArray(item["arr_task"])){
                    item["arr_task"].forEach(itemTask => {
                        ...
                        if(arr.length == 0)
                            this.m_Data["arr_task"].push(itemTask);
                    });
                }
            });
    ...
    OnButtonClick(el){
        ...
        if(value == undefined)
            value = el.closest(".value").attr("value");

        console.log(action, value);
        if(action == "close")
            this.EventCloseEditClass(el);
        else if(action == "save"){
            if(this.m_Data["class_id"])
                this.SaveEdit();
            else
                this.Add();
        }            
            ...
            this.OpenDialogUser();
        else if(action == "deleteUser" && value)
            this.DeleteUser(value);
        else if(action == "addTool")
            this.OpenDialogTool();
        else if(action == "deleteTool" && value)
            this.DeleteTool(value);            
        else if(action == "addTeam")
            this.AddTeam();
        else if(action == "deleteTeam" && value)
            this.DeleteTeam(value);
        else if(action == "editTeam" && value)
    ...
    ValidateDataSave(data){
        var elError = new Array();

        if(!data["name"] || !data["name"].trim())
            elError.push(this.GetElFieldName());
        if(!data["scheme_type_id"])
            elError.push(this.GetElFieldSchemeType());
        if(!data["date_start"])
            elError.push(this.GetElFieldDateStart());
        if(!data["date_end"])
            elError.push(this.GetElFieldDateEnd());
    ...
    AddTeam(){
        if(!this.m_Data["arr_team"])
            this.m_Data["arr_team"] = new Array();

        var length = this.m_Data["arr_team"].length,
            team = (length == 0) ? 1 : this.m_Data["arr_team"][length-1]["team"] + 1;
    ...
    DeleteTeam(teamId){
        this.m_Data["arr_team"] = this.m_Data["arr_team"].filter(item => {
            return teamId != item["team"];
        });
    ...
    DeleteUser(userId){
        this.m_Data["arr_user"] = this.m_Data["arr_user"].filter(item => {
    ...
    DeleteTool(ToolId){
        this.m_Data["arr_tool"] = this.m_Data["arr_tool"].filter(item => {
            return item["arr_tool"] != ToolId;
        });
    ...
    OpenDialogUser(value){
        this.GetElDialogUser().dialog({
            ...
            buttons: [{
                    ...
                    text: "Выбрать",
                    click: () => {
                        var tree = this.GetElTreeUsers().fancytree("getTree");
                        if(tree){
        ...
        this.GetElTreeUsers().fancytree({
            ...
            init: () => {
                if(Array.isArray(this.m_Data["arr_user"])){
                    this.m_Data["arr_user"].forEach((item) =>{
                        var node = this.GetElTreeUsers().fancytree("getNodeByKey", String(item["user_id"]));
                        if(node){
    ...
    OpenDialogTool(value){
        this.GetElDialogTools().dialog({
            ...
            buttons: [{
                    ...
                    text: "Выбрать",
                    click: () => {
                        var arr = this.GetElTableTools().find(`input[checked="checked"]`);
                        $.map(arr, item => {                            
                            var data = $(item).closest("[value]").data();
                            if(Array.isArray(data["arr_task"]))
                                this.m_Data["arr_task"] = this.m_Data["arr_task"].concat(data["arr_task"]);
                        });
    ...
    OpenDialogGeneral(data){
        ...
        this.GetElDialogGeneral().dialog({
            ...
            buttons: [{
                    ...
                    text: "Выбрать",
                    click: () => {
                        var arr = this.GetSelectedTableRow(this.GetElTableGeneral());

                        this.m_Data["arr_user"] = this.m_Data["arr_user"].map(item => {
                            var arrF = arr.filter(itemF => {
                                return itemF["id"] == item["user_id"];
                            });
    ...
    CreateElTableToolsRow(data){
        ...
        el.on("click", event => {
            var elInput = $(event.currentTarget).find("input");
    ...
    GetSelectedTableRow(elTable){        
        return $.map(elTable.find(`input:checked`).closest("[value]"), item => {
            return $(item).data();
        });
    }

    UpdateElTasks(){
        if(!Array.isArray(this.m_Data["arr_task"]))
            return;
    ...
    GetSelectedTasks(){
        var input = this.GetElFieldTools().find(`input:checked`).closest(".value"),
            arr = new Array();
    ...
    UpdateElTeams(){
        if(!Array.isArray(this.m_Data["arr_team"]))
            return;

        this.GetElFieldTeams().find(".value").remove();

        this.m_Data["arr_team"].forEach(item => {
            var elValues = this.GetElFieldTeams().find(".values")
    ...
    UpdateElUsers(){
        ...
        var elValues = this.GetElFieldUsers().find(".values");
        this.m_Data["arr_user"].forEach(item => {
            elValues.append(`
    ...
    UpdateElSchemeType(){
        ...
        this.m_DataSchemeType.forEach(item => {
            var selected = (item["scheme_type_id"] == this.m_Data["scheme_type_id"]) ? `selected="selected"` : "";
